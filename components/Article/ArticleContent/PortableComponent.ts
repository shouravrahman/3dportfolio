...
import { PortableTextComponents } from '@portabletext/react'; // Import the types if you're using @portabletext/react

// Define the types if not using @portabletext/react
interface PortableTextBlock {
  children: React.ReactNode;
  value?: { href?: string };
}

const articePortableText: PortableTextComponents = {
  ...
  block: {
    ...
    h1: ({ children }: PortableTextBlock) =>
      <h1 className="font-semibold text-3xl my-8 leading-tight md:text-4xl">
        {children}
      </h1>
    ,
  ...
  list: {
    ...
    number: ({ children }: PortableTextBlock) => (
      <ol className="my-4 pl-7">{children}</ol>
    ),
  },
  listItem: {
    bullet: ({ children }: PortableTextBlock) => (
  ...
  marks: {
    link: ({ children, value }: PortableTextBlock) => {
      ...
      return !value?.href?.startsWith("/") ? (
        ...
        <Link href={value.href} className="text-appPurple-100 dark:text-appRed-100 text-decoration-underline">
          {children}
        </Link>
      );
    },
    strong: ({ children }: PortableTextBlock) => (
      <strong className="font-semibold">{children}</strong>
    ),
    emphasis: ({ children }: PortableTextBlock) => (
      <em className="font-italic">{children}</em>
    ),
    code: ({ children }: PortableTextBlock) => (
  ...
  types: {
    code: ({ value }: PortableTextBlock) => (
      <CopyableCodeBlock>{value}</CopyableCodeBlock>
    ),
    image: ({ value }: PortableTextBlock) => (
      <ImagePortableCom value={value} />
    ),
  }
};
